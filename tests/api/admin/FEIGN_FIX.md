# Blog-Admin Feign Client 问题修复

## 问题描述

管理后台 API 测试失败，所有通过 blog-admin 调用后端服务的接口都返回"系统繁忙，请稍后重试"错误。

## 诊断结果

通过 `diagnose-feign.ps1` 脚本诊断发现：

1. **直接调用 blog-user 的 admin 端点成功** ✅
   - URL: `http://localhost:8081/admin/users`
   - 说明 blog-user 服务本身工作正常

2. **通过 gateway 调用 blog-admin 失败** ❌
   - URL: `http://localhost:8000/api/v1/admin/users`
   - 错误: "系统繁忙，请稍后重试"
   - 说明 blog-admin 的 Feign 客户端无法调用 blog-user

## 根本原因

**blog-admin 服务缺少 `bootstrap.yml` 配置文件**

- blog-admin 无法正确连接到 Nacos 服务注册中心
- Feign 客户端无法发现 blog-user 服务实例
...
已创建 `blog-admin/src/main/resources/bootstrap.yml`:
...
1. 用户注册和登录
2. ADMIN 角色分配
3. 直接调用 blog-user admin 端点
4. 通过 gateway 调用 blog-admin 端点
...
[STEP 3] Testing direct call to blog-user...
