...
- **Gateway** (`blog-gateway/src/main/resources/application.yml`):
  ```yaml
  jwt:
    secret: ${JWT_SECRET:blog-microservices-jwt-secret-key-must-be-at-least-256-bits}
  ```

- **User Service** (`blog-user/src/main/resources/application.yml`):
  ```yaml
  jwt:
...
已修改 `blog-user/src/main/resources/application.yml`，使用与网关相同的 secret：
...
2. **blog-gateway** (端口 8000) - 如果之前已重启过，可以跳过
   ```bash
   # 停止当前运行的 blog-gateway
   # 重新启动
   mvn spring-boot:run -pl blog-gateway
   ```
...
- StripPrefix=2 移除 `/api/v1`
- 转发到 blog-admin: `GET /admin/users`

## 相关文件

- `blog-user/src/main/resources/application.yml` - 已修改
- `blog-gateway/src/main/resources/application.yml` - 无需修改
