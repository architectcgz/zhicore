...
- `document.hidden = true` 时不再视为“正在读”，前端应立即停止心跳并尽力发送 leave；恢复可见时重新创建新的 `sessionId` 并重新注册
- 当前页面从匿名态切换为登录态时，不升级旧 session，而是主动结束匿名 session，再以登录身份创建新的 session
...
- 心跳只负责维持 session 存活，不承担页面展示刷新职责
- presence 查询单独以更短周期刷新，是为了让“第二个人进入后，第一个人几秒内看到人数变化”，避免必须等待下一次心跳
...
- 同一 `ip + userAgent + postId` 在短窗口内的 session 创建次数限制
- 同一匿名指纹在旧 session 未过期时重复创建，优先覆盖而不是无限新增
- 对明显异常的匿名心跳频率直接丢弃，不进入人数统计

本次不引入复杂风控系统，但应保证不会因匿名刷量或频繁刷新导致 Redis 热点失控。
...
presence 功能至少补充以下监控指标与日志：

- `presence.session.create` 计数
- `presence.session.renew` 计数
- `presence.session.leave` 计数
- `presence.query` 计数
- `presence.query.degraded` 计数
- `presence.cleanup.expired` 计数

日志要求：

- 仅记录必要维度，如 `postId`、`sessionId`、用户是否匿名、处理结果
- 避免在高频心跳路径输出噪声级 info 日志，默认使用 debug 或指标聚合
...
- 页面进入隐藏态时立即 leave，恢复可见时创建新 session
- 匿名登录后，旧匿名 session 被关闭并重建为登录 session
- 卸载时发送注销请求
...
- 浏览器后台节流可能影响隐藏态 leave 的即时性，最终仍需 TTL 兜底收敛
