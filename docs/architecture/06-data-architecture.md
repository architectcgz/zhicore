...
| 版本 | 日期 | 作者 | 说明 |
|------|------|------|------|
| 1.0 | 2026-02-11 | ZhiCore Team | 初始版本 - 数据库选型、缓存策略、数据一致性 |
...
ZhiCore 微服务系统采用 **PostgreSQL** 作为主数据库，**Redis** 作为缓存层，**MongoDB** 用于存储文章内容等非结构化数据与部分文档型权威状态。本文档详细说明数据架构设计、缓存策略和数据一致性保证机制。
...
| **文档数据库** | MongoDB 6 | 文章内容、富文本存储、Ranking inbox/热度状态/历史归档 | 27017 |

### Ranking 数据落点补充

`zhicore-ranking` 是当前比较特殊的模块，它同时使用 MongoDB 和 Redis，但两者职责不同：

- `MongoDB`
  - `ranking_event_inbox`：消费侧 durable inbox
  - `ranking_post_hot_state`：文章热度权威状态，保存浏览/点赞/收藏/评论净计数以及作者、话题、发布时间等元数据
  - 历史榜单归档：保存可追溯的排行快照
- `Redis`
  - 当前总榜、日榜、周榜、月榜查询缓存
  - 浏览去重和浏览分数上限控制
...
这意味着 `Redis` 在 ranking 中不是唯一真相；即使 Redis 丢失，也可以从 MongoDB 权威状态重新生成当前榜单。
...
| 服务 | 数据库名 | 主要表 | 说明 |
|------|---------|--------|------|
| **ZhiCore-user** | `ZhiCore_user` | users, user_follows, user_check_ins | 用户信息、关注关系、签到记录 |
| **ZhiCore-post** | `ZhiCore_post` | posts, post_stats, post_likes, tags | 文章、统计、点赞、标签 |
| **ZhiCore-comment** | `ZhiCore_comment` | comments, comment_stats, comment_likes | 评论、统计、点赞 |
| **ZhiCore-message** | `ZhiCore_message` | conversations, messages | 会话、私信消息 |
| **ZhiCore-notification** | `ZhiCore_notification` | notifications, global_announcements | 通知、公告 |
...
public User getUser(Long userId) {
    ...
    // 空值标记，直接返回
    if (CacheConstants.NULL_MARKER.equals(cached)) {
        return null;
    }
    ...
    if (user != null) {
        ...
        // 缓存空值，短过期时间（60秒）
        redis.setex("user:" + userId, 60, CacheConstants.NULL_MARKER);
    }
...
public void updateUser(User user) {
    ...
    // 3. 发送延迟删除任务（MQ / 调度器），避免进程重启导致任务丢失
    cacheInvalidationPublisher.publishDelayDelete(
        key,
        Duration.ofSeconds(cacheProperties.getSecondaryDeleteDelaySeconds())
    );
}
```

**要点**：
- 延迟补偿删除必须是可持久化任务，不能依赖 `CompletableFuture + Thread.sleep`
- 延迟时间应通过配置注入，而不是硬编码 500ms
- 对热点 key 需要配合幂等删除和重试
...
// Outbox 调度器（事务提交后异步投递）
@Component
public class OutboxEventDispatcher {

    @Scheduled(fixedDelay = 5000)
    public void dispatchPendingEvents() {
        // 拉取 PENDING 事件，发送 MQ，失败重试，超限进入 DLQ
    }
}
...
public class PostPublishedListener implements RocketMQListener<PostPublishedEvent> {
    ...
    public void onMessage(PostPublishedEvent event) {
        idempotentHandler.execute(event.getEventId(), () ->
