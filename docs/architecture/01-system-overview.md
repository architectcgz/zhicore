...
### 消息通知
- 系统通知（由 notification 服务负责生成、存储和未读统计）
- 私信功能（由 message 服务集成 IM 系统实现）
- 消息中心聚合（message 服务聚合私信会话和通知摘要）
- 实时消息推送
...
- **评论服务**: 评论管理、评论点赞、评论回复
- **消息服务**: 私信编排、会话聚合、IM 适配
- **通知服务**: 系统通知生成、通知存储、未读统计
- **搜索服务**: 全文搜索、搜索建议
- **排行服务**: 热门排行、推荐算法
- **上传服务**: 文件上传、文件管理
...
#### 5. 共享模块
- **共享契约模块**: Feign Client 接口、DTO、事件定义
