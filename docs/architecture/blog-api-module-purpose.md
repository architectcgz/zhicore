...
`blog-api` 是一个**共享 API 模块**，用于在微服务之间共享接口定义、DTO 和事件。它不是一个独立运行的服务，而是一个被其他服务依赖的 Maven 模块。

## 为什么需要 blog-api？

在微服务架构中，服务之间需要相互调用。`blog-api` 模块解决了以下问题：

### 1. 服务间调用（Feign Client）

当一个服务需要调用另一个服务时，需要定义 Feign 客户端接口。这些接口定义在 `blog-api` 中，供所有服务共享。

**示例**：`blog-user` 需要调用 `blog-leaf` 服务生成分布式 ID

```java
// blog-api/client/LeafServiceClient.java
...
public class CheckInApplicationService {
    @Autowired
    private LeafServiceClient leafServiceClient;  // 来自 blog-api
...
服务之间传递数据时需要统一的数据结构。`blog-api` 定义了跨服务共享的 DTO。

**示例**：`blog-comment` 需要获取用户信息，`blog-notification` 也需要获取用户信息

```java
// blog-api/dto/user/UserSimpleDTO.java
...
public class CommentApplicationService {
    @Autowired
    private UserServiceClient userServiceClient;  // 来自 blog-api
...
微服务之间通过消息队列（RocketMQ）进行异步通信。`blog-api` 定义了统一的事件格式。

**示例**：用户关注事件

```java
// blog-api/event/user/UserFollowedEvent.java
...
## blog-user 为什么依赖 blog-api？

`blog-user` 服务依赖 `blog-api` 的原因：
...
@Autowired
private LeafServiceClient leafServiceClient;  // 需要 blog-api
...
## 为什么需要扫描 com.blog.api 包？

这就是之前遇到的问题！

```java
@SpringBootApplication(scanBasePackages = {
    "com.blog.user",      // 扫描用户服务自己的代码
    "com.blog.common",    // 扫描公共模块
    "com.blog.api"        // 扫描 API 模块 ← 必须添加！
})
```

**原因**：`blog-api` 中的 `LeafServiceFallbackFactory` 是一个 `@Component`，需要被 Spring 扫描并创建 Bean。

```java
// blog-api/client/fallback/LeafServiceFallbackFactory.java
...
└─────────────────────────────────────────────────────────────┘
        ...
        │                   │                   │
┌───────▼────────┐  ┌──────▼───────┐  ┌───────▼────────┐
│  blog-user     │  │ blog-comment │  │  blog-post     │
...
- `LeafServiceFallbackFactory` 在 `blog-api` 中（基础设施服务，所有服务共用）
- `UserServiceFallbackFactory` 在 `blog-comment` 中（业务服务，降级策略可能不同）
- `PostServiceFallbackFactory` 在 `blog-comment` 中（业务服务，降级策略可能不同）

### 3. 必须扫描 com.blog.api 包
- 所有使用 `blog-api` 的服务都必须在 `@SpringBootApplication` 中添加 `"com.blog.api"` 到 `scanBasePackages`
- 否则 `@Component` 注解的类（如 `LeafServiceFallbackFactory`）无法被 Spring 发现

## 总结

`blog-api` 是微服务架构中的**契约层**：
