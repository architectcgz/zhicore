...
本文档描述了 Blog 微服务系统中的文件上传架构，包括架构演进历史、服务职责划分、上传和删除流程、支持的文件类型以及最佳实践。

### 核心特点

- **统一入口**: 前端通过 blog-upload 服务统一上传文件
- **服务解耦**: 后端通过 BlogUploadClient 调用文件删除接口
...
在早期架构中，我们在 `blog-common` 模块中定义了 `FileUploadService` 接口，各个业务服务（blog-user、blog-post、blog-comment）都实现了这个接口。这种设计存在以下问题：
...
- **blog-user**: 核心职责是用户管理，不应该关心文件上传的实现细节
- **blog-post**: 核心职责是文章管理，不应该关心文件上传的实现细节
- **blog-comment**: 核心职责是评论管理，不应该关心文件上传的实现细节
...
通过引入独立的 `blog-upload` 服务，我们解决了上述所有问题：

#### 优势 1: 代码复用

所有文件上传逻辑集中在 `blog-upload` 服务中，避免了代码重复：

```java
// blog-upload 服务中的统一实现
@Service
...
- **blog-upload**: 专注于文件上传和管理
- **业务服务**: 专注于业务逻辑，通过 BlogUploadClient 调用文件删除接口

#### 优势 3: 统一管理

所有文件上传相关的配置和逻辑都在 `blog-upload` 服务中：
...
**职责**:
- 管理文件 URL 与业务实体的关联关系
- 在业务实体删除时调用 BlogUploadClient 删除文件
...
graph TB
    ...
    subgraph "文件服务层"
        UploadService[blog-upload<br/>内部服务]
    ...
    UserService -.->|3. 删除文件<br/>BlogUploadClient| UploadService
    PostService -.->|3. 删除文件<br/>BlogUploadClient| UploadService
    CommentService -.->|3. 删除文件<br/>BlogUploadClient| UploadService
    
    %% blog-upload 调用 file-service-client
...
sequenceDiagram
    ...
    participant Gateway as blog-gateway
    participant Upload as blog-upload
...
前端通过 API 网关调用 blog-upload 服务的上传接口：
...
验证通过后，blog-upload 调用 file-service-client 上传文件：
...
```mermaid
sequenceDiagram
    participant PostService as blog-post
    participant UploadClient as BlogUploadClient
    participant Upload as blog-upload
    participant FileClient as file-service-client
    participant FileService as File Service
    participant MinIO as MinIO
...
public class PostApplicationService {
    ...
    private final BlogUploadClient blogUploadClient;
    ...
    public void deletePost(String postId) {
        ...
        for (String fileId : fileIds) {
            try {
                blogUploadClient.deleteFile(fileId);
...
#### 2. 通过 BlogUploadClient 删除文件

BlogUploadClient 是一个 Feign Client，定义了删除文件的接口：
...
#### 3. blog-upload 转发删除请求

blog-upload 服务接收到删除请求后，转发给 file-service-client：
...
前端应该统一调用 blog-upload 服务的上传接口，而不是调用各个业务服务的接口：

```javascript
// ✅ 推荐：统一调用 blog-upload 服务
const uploadImage = async (file) => {
  const formData = new FormData()
  formData.append('file', file)
...
后端删除文件时，应该通过 BlogUploadClient 调用 blog-upload 服务：

```java
// ✅ 推荐：使用 BlogUploadClient
@Service
@RequiredArgsConstructor
public class PostApplicationService {
...
public class HealthCheckController {
    ...
    public Map<String, Object> health() {
        ...
        health.put("service", "blog-upload");
...
### Q2: 为什么后端要通过 BlogUploadClient 删除文件？

**原因**:
1. **服务解耦**: 业务服务不直接依赖 file-service-client
2. **统一管理**: 所有文件操作都通过 blog-upload 服务，便于统一管理和监控
3. **降级策略**: 可以在 BlogUploadClient 中实现降级策略，提高系统可用性
4. **日志记录**: 在 blog-upload 服务中统一记录文件操作日志
...
Blog 文件上传架构通过引入独立的 `blog-upload` 服务，实现了以下目标：

1. **代码复用**: 避免在每个业务服务中重复实现文件上传逻辑
2. **职责清晰**: 文件上传由专门的服务处理，业务服务专注于业务逻辑
3. **统一管理**: 所有文件上传相关的配置和逻辑都在 blog-upload 服务中
4. **简化前端**: 前端只需要调用一个统一的上传接口
5. **服务解耦**: 业务服务通过 BlogUploadClient 调用文件删除接口，不直接依赖 file-service-client
