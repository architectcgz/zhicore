...
public class CommentCommandService {
    ...
    private final CommentIntegrationEventPort commentIntegrationEventPort;
    ...
    public Long createComment(Long authorId, CreateCommentCommand request) {
        ...
        transactionTemplate.executeWithoutResult(status -> {
            ...
            commentIntegrationEventPort.publish(new CommentCreatedIntegrationEvent(
                    newEventId(),
                    Instant.now(),
                    comment.getId(),
                    comment.getPostId(),
    ...
    public void updateComment(Long currentUserId, Long commentId, UpdateCommentCommand request) {
        ...
        transactionCommitSignal.afterCommit(() -> evictCommentDetail(commentId));
    ...
    public void deleteComment(Long currentUserId, boolean isAdmin, Long commentId) {
        ...
        commentIntegrationEventPort.publish(new CommentDeletedIntegrationEvent(
                newEventId(),
                Instant.now(),
                commentId,
                comment.getPostId(),
        ...
        transactionCommitSignal.afterCommit(() -> {
            if (comment.isReply()) {
                try {
                    ...
                    log.warn("Redis 回复计数更新失败: {}", e.getMessage());
                }
            }
            ...
            try {
                ...
                log.warn("Redis 评论计数更新失败: {}", e.getMessage());
            }

            evictCommentDetail(commentId);
        });
    ...
    private void evictCommentDetail(Long commentId) {
        try {
