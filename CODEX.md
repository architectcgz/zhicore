...
- 在 `zhicore-microservice` 内写代码时，必须先遵循 [docs/architecture/code-style-guide.md](/home/azhi/workspace/projects/zhicore-microservice/docs/architecture/code-style-guide.md) 的约束。
- 若 `CODEX.md` 与个人习惯、历史写法或临时便利做法冲突，优先按代码风格文档执行。
- 做设计、实现、重构、补测试时，都要先检查是否违反该文档中的分层边界、依赖方向、事务边界、幂等、缓存、日志与测试规范。
...
- 默认串行执行高负载验证，不要把 Maven reactor 测试、前端构建、浏览器回归或其他重任务并发叠加运行。
- 默认不要主动增加并行度：不加 Maven `-T`，也不额外提升测试 worker/threads 数；如必须并发，先收敛范围并说明原因。
- 遇到“测试卡死、CPU 打满、内存吃紧”迹象时，立即停止叠加任务，拆成更小的分批验证。
