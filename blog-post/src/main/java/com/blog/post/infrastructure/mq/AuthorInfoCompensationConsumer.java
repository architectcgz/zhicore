...
import com.blog.api.client.UserServiceClient;
import com.blog.api.dto.user.UserSimpleDTO;
import com.blog.api.event.post.AuthorInfoCompensationEvent;
...
public class AuthorInfoCompensationConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String message) {
        try {
            ...
            // 解析消息
            AuthorInfoCompensationEvent event = objectMapper.readValue(
                message, 
                AuthorInfoCompensationEvent.class
            );
            
            log.info("开始处理作者信息补偿: postId={}, userId={}, createdAt={}", 
                event.getPostId(), event.getUserId(), event.getCreatedAt());
            ...
            if (!post.hasDefaultAuthorInfo()) {
                log.info("文章作者信息已更新，跳过补偿: postId={}, currentOwnerName={}", 
                    event.getPostId(), post.getOwnerName());
                return;
            }
