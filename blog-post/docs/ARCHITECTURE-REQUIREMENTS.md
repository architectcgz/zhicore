...
评论详情
├─ 原因: 评论内容变化频繁，冗余成本高
└─ 方式: 前端调用 blog-comment 服务

用户详细资料
├─ 原因: 资料字段多，冗余维护成本高
└─ 方式: 前端调用 blog-user 服务

点赞状态（当前用户是否点赞）
├─ 原因: 个性化数据，无法冗余
└─ 方式: 前端调用 blog-like 服务
```
...
消费者:
├─ blog-search: 创建ES索引
├─ blog-notification: 通知关注者（如果已发布）
├─ blog-ranking: 初始化排行数据
└─ blog-stats: 初始化统计数据
...
消费者:
├─ blog-notification: 通知关注者
├─ blog-feed: 推送到关注者Feed流
└─ blog-ranking: 加入热门榜单
...
消费者:
├─ blog-post: 更新 post_stats.like_count
└─ blog-ranking: 更新热门榜单
...
消费者:
├─ blog-post: 更新 post_stats.comment_count
└─ blog-notification: 通知文章作者
...
消费者:
├─ blog-post: 更新 posts.owner_name, owner_avatar_id
└─ 其他服务: 更新各自的用户信息冗余
...
推荐文章
├─ 来源: blog-recommendation 服务（算法推荐）
├─ 实现: 协同过滤 + 内容推荐
└─ 缓存: Redis List (个性化推荐)
...
├─ blog-post-write: 写操作
├─ blog-post-read: 读操作
├─ blog-post-stats: 统计服务
└─ blog-post-search: 搜索服务
