# Implementation Plan: ZhiCore-migration 重构为 ZhiCore-ops

## Overview

将 ZhiCore-migration 模块重构为 ZhiCore-ops 运维服务模块，移除已废弃的 Flyway 和数据迁移功能，保留 CDC 和灰度发布功能。

## Tasks

- [x] 1. 重命名模块目录和基础配置
  - [x] 1.1 使用 Git 重命名模块目录
    - 执行 `git mv ZhiCore-migration ZhiCore-ops`
    - _Requirements: 1.1_
  
  - [x] 1.2 更新根 pom.xml 中的模块引用
    - 将 `<module>ZhiCore-migration</module>` 改为 `<module>ZhiCore-ops</module>`
    - _Requirements: 1.5, 9.1_
  
  - [x] 1.3 更新模块 pom.xml
    - 将 `<artifactId>ZhiCore-migration</artifactId>` 改为 `<artifactId>ZhiCore-ops</artifactId>`
    - 将 `<name>ZhiCore Migration Service</name>` 改为 `<name>ZhiCore Ops Service</name>`
...
- [ ] 5. 更新 Java 包名
  ...
  - [ ] 5.2 批量更新包名
    - 使用 IDE 重构功能将 `com.ZhiCore.migration` 重命名为 `com.ZhiCore.ops`
  ...
  - [ ] 5.3 验证包引用
    ...
    - 检查所有 import 语句
    - 确保没有遗漏的 `com.ZhiCore.migration` 引用
    - _Requirements: 5.6, 6.5_

- [ ] 6. 更新配置文件
  - [ ] 6.1 更新 application.yml
    - 将 `spring.application.name` 改为 `ZhiCore-ops`
    - 删除已注释的 `migration` 配置段
    - 删除 `mybatis-plus` 配置段（如果存在）
    - 将 `logging.level.com.ZhiCore.migration` 改为 `logging.level.com.ZhiCore.ops`
    - 确保保留 `cdc` 配置段
    - 确保保留 `gray` 配置段
    - _Requirements: 7.1, 7.2, 7.4, 7.6, 5.5, 6.4_
  
  - [ ] 6.2 更新 bootstrap.yml（如果需要）
    - 检查是否有对 ZhiCore-migration 的引用
    - 如有，更新为 ZhiCore-ops
    - _Requirements: 1.4_

- [ ] 7. 检查和更新项目引用
  - [ ] 7.1 检查其他模块的依赖
    - 搜索其他模块的 pom.xml 中是否依赖 ZhiCore-migration
    - 如有，更新 artifactId 为 ZhiCore-ops
    - _Requirements: 9.2_
  
  - [ ] 7.2 检查文档引用
    - 搜索 docs 目录中对 ZhiCore-migration 的引用
    - 更新为 ZhiCore-ops
    - _Requirements: 9.3_
  
  - [ ] 7.3 检查脚本引用
    - 搜索 scripts 和 tests 目录中对 ZhiCore-migration 的引用
    - 更新为 ZhiCore-ops
    - _Requirements: 9.4_
  
  - [ ] 7.4 检查 Docker 配置
    - 检查 docker-compose 文件中是否有 ZhiCore-migration 引用
    - 如有，更新为 ZhiCore-ops
    - _Requirements: 9.5_

- [ ] 8. 第一次验证 - 编译测试
  - [ ] 8.1 清理并编译模块
    - 执行 `mvn clean compile -pl ZhiCore-ops`
    - 确保编译成功，无错误
    - _Requirements: 10.1_
...
- [ ] 9. 第二次验证 - 打包测试
  - [ ] 9.1 打包模块
    - 执行 `mvn clean package -pl ZhiCore-ops`
    - 确保打包成功，生成 jar 文件
    - _Requirements: 10.2_
  
  - [ ] 9.2 检查 jar 文件
    - 验证 jar 文件名为 `ZhiCore-ops-1.0.0-SNAPSHOT.jar`
    - 检查 jar 文件大小是否合理
    - _Requirements: 10.2_
...
- [ ] 10. 第三次验证 - 启动测试
  - [ ] 10.1 启动应用
    - 确保 Redis 服务已启动
    - 执行 `java -jar ZhiCore-ops/target/ZhiCore-ops-1.0.0-SNAPSHOT.jar`
    - 或使用 IDE 运行 OpsApplication
    - _Requirements: 10.3_
  
  - [ ] 10.2 检查启动日志
    - 确认应用名显示为 ZhiCore-ops
    - 确认没有启动错误
...
- [ ] 12. 最终检查和清理
  ...
  - [ ] 12.2 更新相关文档
    - 更新项目 README（如果提到 ZhiCore-migration）
    - 更新架构文档
    - 添加 ZhiCore-ops 模块说明
    - _Requirements: 9.3_
